...
export class Kernel {
    ...
    static DI_PARAM_DEVELPOMENT = 'isDevelopment';
    ...
    constructor(
        ...
        container.setParameter(Kernel.DI_PARAM_DEVELPOMENT, this.isDevelopment);
        ...
        for (let bundle of this.bundles) {
            if (types.isPromise(container.invoke(bundle, 'init'))) {
                container.invoke(Logger).debug(`Warning: ${bundle.bundleName}.init() is asynchronous but bundle initialization doesn\'t support serial execution for async`);
            }
    ...
    get isProduction() {
        return this.env == 'production' || this.env == '';
    }
     ...
     * Kernel.bootAndRun({ parameters: { twigCache: false } }, "cli");
     * ```
     * @param configFile Configuration file
     * @param params Run parameters
     */
    async bootAndRun( configFile?: string | object, ...params: any[]) {
    ...
    static async createFullStack(bundles?: Bundle[], env?: string): Promise<Kernel> {
        let { HttpBundle , CliBundle, TwigBundle, TypeORMBundle, MessageBundle, MongooseBundle, MongoBundle, SecurityBundle, MakerBundle, GraphQlBundle, AssetsBundle, OpenApiBundle } = await import('../bundle');
            
        return new Kernel(env, [ new HttpBundle, new CliBundle, new TwigBundle, new TypeORMBundle, new MongooseBundle, new MessageBundle, new MongoBundle, new SecurityBundle, new MakerBundle, new GraphQlBundle, new AssetsBundle, new OpenApiBundle, ...(bundles ?? []) ]);
    }
