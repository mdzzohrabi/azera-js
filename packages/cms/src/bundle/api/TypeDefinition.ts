 ...
 */
export class TypeDefinition extends Array<string | (() => (string | Promise<string>))> {
    ...
    async getTypeDefinition() {
        let defs: string[] = [];
        for (let def of this) {
            if (typeof def == 'function') defs.push(await def());
            else if (typeof def == 'string') defs.push(def);
        }
